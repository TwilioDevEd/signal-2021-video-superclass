...
def find_or_create_room():
    ...
    try:
        # Try to fetch an in-progress Video room with the name that's
        # the value of the global ROOM_NAME variable
        ...
        room = twilio_client.video.rooms.create(
            ...
            # or `p2p` for Peer-to-Peer rooms)
            type="group",
        )
    # Print how many participants the room has
    print(f"{room.unique_name} has {len(room.participants.list())}"
        " participants in it.")
